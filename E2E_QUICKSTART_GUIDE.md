...
| `proxmox_api_password` | SSH password (via `ansible_password` in inventory) | `<your-proxmox-password>` |
...
| `proxmox_root_password` | Root password for Proxmox | `<your-proxmox-password>` |
...
| `bcm_password` | BCM root password (used by cm-master-install) | `<your-bcm-password>` |
...
## Stage 2: `make bcm-vm-create-api`
...
| `bcm_password` | BCM root password (for SSH key auth) | `<your-bcm-password>` |
...
## Stage 6: `make validate`

**Playbook**: `playbooks/07-validate.yml`
**Role**: `validate`
**Hosts**: `localhost`

### What it does

Runs a 41-point validation across both the BCM head node and the Kairos compute node. Tests connectivity, services, networking, DNS, boot config, disk layout, and cloud config. Results are written to `build/validate.log`.

#### BCM Head Node checks (21 checks)

| Category | Checks |
|----------|--------|
| **Connectivity** | SSH access, ping |
| **Services** | cmd, dhcpd, named (DNS), nfs-server, rsyncd (873), HTTP server (8888) |
| **Network** | eth0 IP matches `bcm_internal_ip`, eth1 IP matches `bcm_external_ip`, default gateway |
| **DNS** | External DNS resolution (google.com), internet access (HTTPS) |
| **Cluster** | Head node UP in cmsh, node001 registered, node001 IP correct, category is `kairos`, kairos-installer image exists, raw image exists on disk |

#### Kairos Compute Node checks (20 checks)

| Category | Checks |
|----------|--------|
| **Connectivity** | SSH access (via BCM jump), ping |
| **OS** | OS release, Kairos version, kairos-agent version, kernel |
| **Network** | IP address, gateway, DNS resolver, external DNS resolution, internet access |
| **Services** | stylus-agent, Palette registration |
| **Boot** | `net.ifnames=0` in cmdline, Kairos boot chain (`rd.cos` or `rd.immucore`) |
| **Disk** | COS_OEM, COS_RECOVERY, COS_STATE, COS_PERSISTENT partitions, root immutable (read-only), free space |
| **Config** | OEM yaml files present in `/oem/` |

### Required configuration

| Variable | Purpose | Default |
|----------|---------|---------|
| `bcm_connect_ip` | BCM SSH target | `10.100.0.200` |
| `bcm_password` | BCM root password | `<your-bcm-password>` |
| `bcm_internal_ip` | Expected eth0 IP | `10.100.0.200` |
| `bcm_external_ip` | Expected eth1 IP | `10.100.0.201` |
| `bcm_external_gateway` | Expected gateway | `10.100.0.1` |
| `bcm_external_dns` | Expected DNS | `10.100.0.1` |
| `kairos_vm_mac` | Compute node MAC (for ARP lookup) | `52:54:00:00:02:01` |

### Output

Results are displayed in ansible output and saved to `build/validate.log`:
...
== BCM Head Node (10.100.0.200) ==

-- Connectivity --
  [PASS] BCM SSH — 10.100.0.200
  [PASS] BCM ping
...
Any `[FAIL]` causes the playbook to exit with a non-zero return code. `[WARN]` results include a description explaining what's expected and why it might be warning.

### What can go wrong

- **Cannot SSH to BCM** — check `bcm_connect_ip` and `bcm_password`
- **Cannot SSH to Kairos** — the node SSH key changes on every reinstall. The script tries both key-based and password auth.
- **DNS/Internet failures** — BCM's named needs forwarders set (`deploy-dd` configures this). BCM needs IP forwarding + NAT if both NICs share a subnet.
- **stylus-agent not running** — Palette agent may take a minute to start on first boot
...
make proxmox-up            # Stage 0: Proxmox testbed + BCM VM shell
make bcm-prepare           # Stage 1: Remaster BCM ISO
make bcm-vm-create-api     # Stage 2: Install BCM, wait for poweroff, boot from disk
make kairos-build          # Stage 3: Build Kairos ISO + raw disk image
make deploy-dd             # Stage 4: Upload image, configure PXE on BCM
make kairos-vm-create      # Stage 5: PXE boot Kairos, dd, poweroff, boot from disk
make validate              # Stage 6: 41-point validation
```
